...
* **Timestamp 정밀도 사양 통일**: PostgreSQL의 `TIMESTAMP` 타입은 최대 6자리(Microseconds)의 소수점 이하 초 정밀도를 지원합니다. 불필요하게 9자리(`TIMESTAMP(9)`)로 선언 시 생성 오류를 유발하거나 드라이버 호환성 문제가 생길 수 있으므로, 메타데이터 테이블 정의 시 타임스탬프 필드 정밀도를 `TIMESTAMP(6)`으로 통일하여 마이그레이션 스크립트에 반영합니다.
