...
### Decision
* **DLQ (Dead Letter Queue) 테이블 패턴**을 적용합니다.
* 특정 장소 수집 중 오류가 발생하면, 해당 아이템의 수집만 skip하고 에러 정보(실패한 contentId, 에러 메시지, 타임스탬프)를 별도 에러 로그 테이블(`ETL_FAILURE_LOG` 등)에 기록한 뒤 다음 데이터 처리를 계속 진행합니다.
